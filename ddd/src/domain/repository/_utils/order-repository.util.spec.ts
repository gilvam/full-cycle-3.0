...
describe('OrderRepositoryUtil unit tests', () => {
	it('should redo order list', () => {
		const orderItems = iOrderList.map((it) => {
			const items = it.items.map((it) => new OrderItem(it.id, it.name, it.price, it.quantity, it.productId));
	...
	it('should redo order', () => {
		...
		const orderItems = iOrder.items.map((it) => new OrderItem(it.id, it.name, it.price, it.quantity, it.productId));
