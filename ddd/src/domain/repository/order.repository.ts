...
export default class OrderRepository implements IOrderRepository {
	...
	private async checkItemsUpdate(newOrder: Order, oldOrder: Order): Promise<void> {
		const itemsToChange = newOrder.items.filter((item) =>
			oldOrder.items.find((itemOld) => itemOld.id === item.id && ObjectUtils.notEquals<OrderItem>(item, itemOld))
		);
	...
	private async checkItemsAdd(newOrder: Order, oldOrder: Order): Promise<void> {
		const itemsToAdd = newOrder.items.filter((item) => !oldOrder.items.find((i) => i.id === item.id));
	...
	private async checkItemsDelete(newOrder: Order, oldOrder: Order): Promise<void> {
		const itemsToDelete = oldOrder.items.filter((item) => !newOrder.items.find((i) => i.id === item.id));
